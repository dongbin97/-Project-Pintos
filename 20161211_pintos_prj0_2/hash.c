...
void hash_square(struct hash_elem* e, void* aux) { //set data as square of current data
    struct hash_item* item = hash_entry(e, struct hash_item, elem);
    int data = item->data;
    item->data = data * data;
}

void hash_triple(struct hash_elem* e, void* aux) { //set triple as square of current data
    struct hash_item* item = hash_entry(e, struct hash_item, elem);
    int data = item->data;
    item->data = data * data * data;
}

void hash_item_free(struct hash_elem* e, void* aux) { //free hash_item bt hash_elem in it
    struct hash_item* item = hash_entry(e, struct hash_item, elem);
    free(item);
}

unsigned hash_int_2(int i) { // new hash function
    return i % 13;
}