...
/* ====== newly inserted ====== */
struct bitmap* bitmap_expand(struct bitmap* bitmap, int size) { //expand bitmap size and fill 0
    size_t cur_size = bitmap_size(bitmap);
    size_t new_size = cur_size + size;

    struct bitmap* new_b = bitmap_create(new_size);

    for (size_t i = 0; i < cur_size; i++) 
        bitmap_set(new_b, i, bitmap_test(bitmap, i));
