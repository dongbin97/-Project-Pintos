...
process_execute (const char *file_name) 
{
  ...
  char* command = get_command(file_name);
  ...
  while (!thread_current()->load_finish) thread_yield();
...
start_process (void *file_name_)
{
  ...
  thread_current()->parent->load_finish = true;
...
process_wait (tid_t child_tid UNUSED) 
{
    ...
    struct list_elem* iter = list_begin(&(cur->child_list));
    ...
    if (flag) {
        while (!child->ready_to_exit) thread_yield();
        exit_status = child->exit_status;
        list_remove(iter);
        child->deleted_from_parent = true;
        return exit_status;
    }
    else 
        return -1;
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  int argc = set_argv((char*)file_name, &argv);
  set_stack(argc, &argv, esp);
...
void set_stack(int argc, char** argv, void** esp) {
    int mem_use = 0;
    char** address = (char**)malloc(sizeof(char*) * argc);
